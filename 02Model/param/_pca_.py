...
class PCA():
    ...
    # def __init__(self):
        with torch.no_grad():
            ...
            val = torch.load("val.t")
            self.eigenvalues = (val/torch.sum(val)).float().cuda()
            self.eigenvectors = torch.load("vec.t").float().cuda()
    ...
    # def get_components(self, x, y, train):
        with torch.no_grad():
            ...
            components = [torch.nn.functional.conv2d(
                e, weight.view(3, 1, 11, 11), padding=5, groups=3
            ) for weight in self.eigenvectors]
            components = [torch.nn.functional.conv2d(
                comp, weight.view(3, 1, 11, 11), padding=5, groups=3
    ...
    def generate_img(self, components, x):
        with torch.no_grad():
            ...
            e = torch.sum(torch.stack(components), 0)
            print(e.shape)
            return e + x
